...
int compute_triangle(matrix_opt_t *matrix_opt)
{
	...
	for (int i = 0; i < result->infile_num; i++)
		tmp_ct_list[i] = result->sketch_index[i + 1] - result->sketch_index[i];
	...
	if (matrix_opt->metric == 0)
		...
		err(errno, "%s(): matrix_opt->metric should be 0 or 1", __func__);
	// read qry sketch and index
	khash_t(kmer_set) **h_arr = (khash_t(kmer_set) **)malloc(result->infile_num * sizeof(khash_t(kmer_set) *));
	uint32_t *enrolled_qry = calloc(result->infile_num, sizeof(uint32_t));
	uint32_t enrolled_num = 0;
...
#pragma omp parallel for shared(overlap_kmer_cnt, tmp_distance) num_threads(matrix_opt->p)
		...
		{ // enrolled qry num
			if (stop_flag)
				continue; // Skip if stop condition is met
			...
			{
				if (kh_get(kmer_set, tmp_ht, result->comb_sketch[qi]) != kh_end(tmp_ht))
...
#pragma omp atomic write
		...
		for (uint32_t er_qn = 0; er_qn < enrolled_num; er_qn++)
			fprintf(output, "\t%lf", tmp_distance[er_qn]);
		if (matrix_opt->d)
			fprintf(output, "\t%lf", 0.0);
		fprintf(output, "\n");
		// hash
		khash_t(kmer_set) *h = kh_init(kmer_set);
		int ret;
		for (uint32_t qi = result->sketch_index[qn]; qi < result->sketch_index[qn + 1]; qi++)
			kh_put(kmer_set, h, result->comb_sketch[qi], &ret);
		h_arr[enrolled_num] = h;
		enrolled_qry[enrolled_num++] = qn;
	} // loop qn end

	if (matrix_opt->gl[0] != '\0')
	{ // output genome selection code
		FILE *glout;
	...
	// clean
	for (int er_qn = 0; er_qn < enrolled_num; er_qn++)
		kh_destroy(kmer_set, h_arr[er_qn]);
	if (output != stdout)
		fclose(output);
...
#pragma omp parallel for num_threads(matrix_opt->p)
		for (int qn = 0; qn < qry_result->infile_num; qn++)
		{
			...
			{
				if (kh_get(kmer_set, h, qry_result->comb_sketch[qi]) != kh_end(h))
					overlap_kmer_cnt[qn]++;
			} // for qi
		} // for qn
